    ...
    <div class="center_content">
			...
			?>
 			...
 			<?php
                                            ...
                                            FROM View_Cockboard ORDER BY Add_Datetime DESC";
                                            $result = mysqli_query($db, $query);
			...
			?>
			 ...
			 <?php
			                                ...
			                                $query = "SELECT * FROM `Watch` WHERE Email='{$_SESSION['email']}' AND Watched_Email='{$opened_email}' AND Title='{$opened_title}'";
                                            $result = mysqli_query($db, $query);
			 ...
			 <?php echo "This CorkBoard has " . $row['number_watchers'] . " watchers.";?>
			 			 <form   method="POST" action="watch.php" id="watch"  target="hiddenFrame"> 		
						 <input  type='hidden' name='Watched_Email' value="<?php echo $opened_email;?>">
						 <input  type='hidden' name='Watched_Title' value="<?php echo $opened_title;?>">
						 <input  type='hidden' name='watch_status' value="<?php echo $watch_status;?>">
						
						<?php
						if ($opened_email==$_SESSION['email']){
						print '<button type="submit" disabled form="watch" value="Watch"> Watch it !</button>';
						}
						elseif ($watch_status==1){
						print '<button type="submit"  form="watch" value="Unwatch"> Unwatch it!</button>';
						}	
						else print '<button type="submit"  form="watch" value="Watch">Watch it !</button>';
